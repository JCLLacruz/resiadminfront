import { Box, Container, Divider, Heading, Text, Spinner, Button, Image, Modal, ModalOverlay, ModalContent, ModalBody, useDisclosure, Accordion, AccordionItem, AccordionButton, AccordionIcon, AccordionPanel } from '@chakra-ui/react';
      ...
      <Container maxW='container.xl' paddingBottom={'10rem'} overflowY={'auto'}>
        <Box display={'flex'} gap={'1rem'} justifyContent={'end'} marginBottom={'1rem'} >
          ...
          <Box margin={0} padding={0} display={'flex'} flexDirection={'column'} gap={'1rem'}>
            {currentUser?.role === 'superadmin' && <Button backgroundColor={'red'} _hover={{ bg: 'red' }} onClick={() => setIsAlertVisible(!isAlertVisible)}>{trashIcon}</Button>}
        ...
        <Box display={'flex'} gap={'2rem'}>
          ...
          <Box>
            ...
            <Text mb={'1rem'} paddingLeft={'1rem'}>
              Teléfono: {user.telephonnumber}
            </Text>
            <Text mb={'1rem'}>Cumpleaños: {new Date(user.birthday).toLocaleDateString()}</Text>
            <Text mb={'1rem'}>Rol: {user.role}</Text>
          </Box>
        </Box>
